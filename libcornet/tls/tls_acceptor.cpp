...
coroutines::CoroutineAwaiter<bool> TlsAcceptor::read_client_hello_record(
...
coroutines::CoroutineAwaiter<bool> TlsAcceptor::read_client_finished_record(
        RecordLayer& record_layer, crypto::TlsHandshake& tls_handshake, record::Parser& parser )
{
    ...
    if( handshake_type != record::HandshakeType::FINISHED )
        throw std::runtime_error( "TlsAcceptor::read_client_finished_record got unexpected handshake record "
                                  + std::to_string(static_cast<uint8_t>(handshake_type) ) );
    ...
    if( err )
        throw std::runtime_error( "TlsAcceptor::read_client_finished_record() failed parse Finished message" );
...
uint32_t TlsAcceptor::produce_server_hello_record(
...
uint32_t TlsAcceptor::produce_encrypted_extensions_record(
        TlsReadBuffer& buffer, crypto::TlsHandshake& tls_handshake )
{
...
uint32_t TlsAcceptor::produce_certificate_record( TlsReadBuffer& buffer, crypto::TlsHandshake& tls_handshake )
{
...
uint32_t TlsAcceptor::produce_certificate_verify_record(
        TlsReadBuffer& buffer, crypto::TlsHandshake& tls_handshake )
{
...
uint32_t TlsAcceptor::produce_server_finished_record( TlsReadBuffer& buffer, crypto::TlsHandshake& tls_handshake )
{
        ...
        Poller& poller, sockaddr_in6* peer_addr, KeyStore* keys_store )
{
    ...
    printf( "TlsServer async_session() got tcp socket\n" );
